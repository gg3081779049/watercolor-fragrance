<template>
    <div class="right-toolbar" ref="right-toolbar">
        <el-tooltip :content="`${show.search ? '隐藏' : '显示'}搜索栏`" placement="top" effect="light" :hide-after="0"
            v-if="show.hasOwnProperty('search')">
            <el-button @click="show.search = !show.search">
                <svg-icon :icon="`zoom-${show.search ? 'out' : 'in'}`" />
            </el-button>
        </el-tooltip>
        <el-tooltip content="列设置" placement="top" effect="light" v-if="show.hasOwnProperty('columns')">
        ...
        <slot />
        <el-tooltip content="刷新" placement="top" effect="light" v-if="$attrs.onRefresh">
