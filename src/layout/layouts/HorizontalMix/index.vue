    ...
    <el-container>
        <el-header v-show="!tabFullscreen">
            <Navbar>
                ...
                <Menu :data="sidebarRouteTree.find(item => item.path === $route.path.split('/')[1]).children"
            ...
            <el-aside v-show="!tabFullscreen">
                <Sidebar :style="{ width: `${collapse ? 54 : 90}px` }">
                    <el-menu class="sidebar-menu" :default-active="$route.path.split('/')[1]">
                        <el-menu-item v-for="item in sidebarRouteTree" :key="item.path" :index="item.path"
...
export default {
    ...
    computed: {
        ...mapState(useAppStore, ["collapse", "title", "tabFullscreen"]),
        ...mapState(useRouteStore, ["sidebarRouteTree"]),
        ...mapState(useSettingsStore, ["headerHeight", "fixedHeader", "showTabs", "tabsHeight"])
    },
    methods: {
        handleClick(route) {
            route = route.hasOwnProperty('children') ? this.findFirstRoute(this.sidebarRouteTree.find(item => item.path === route.path).children) : route
